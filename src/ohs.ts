...
export class OHSClient {
	...
	private async queryState(retries = 3, attempt = 0): Promise<OHSSensorState[]> {
		try {
	...
	private static parseState(inputHTML: string) {
		...
		const tidied = inputHTML.split('\n').find(line => line.indexOf('body onload=') !== -1);

		if (!tidied) {
			throw new Error('Unable to find body onload= line!');
		}
		...
		for (const line of fields) {
			...
			//const zone = line[5]; // unused
			const lastOK = OHSClient.parseTime(line[6]);
			//const lastAlarm = OHSClient.parseTime(line[7]) || 0; // unused
